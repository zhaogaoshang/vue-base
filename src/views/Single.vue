  ...
  methods: {
    ...
    async save () {
      try {
        ...
        if (res.result.levels === 2) {
          // 去查询它的子集
          const res1 = await this.$http.post(this.$apis.api_category_listByParentId, {parentUuid: res.result.uuid})
          res.result.children = res1.result ? res1.result : []
        }
